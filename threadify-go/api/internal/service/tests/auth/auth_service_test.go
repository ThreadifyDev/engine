...
func TestAuthService_Signup(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(deps *common.MockedDeps, pool *authmocks.MockDBPool, tx *authmocks.MockTx) {
				deps.UserRepo.EXPECT().FindByEmail(gomock.Any(), email).Return(&domain.User{ID: "existing"}, nil)
			},
			wantErr: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := deps.NewAuthService(pool, authClient, nil, nil, []byte(testEncryptionKey))
...
func TestAuthService_Login(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			req:  &domain.LoginCmd{Email: email, Password: password},
			setupMock: func(deps *common.MockedDeps, authClient *sharedmocks.MockAuthClient) {
				deps.UserRepo.EXPECT().FindByEmail(gomock.Any(), email).Return(&domain.User{ID: "user_123", AuthUserID: &authUserID, EmailVerified: true, Email: email}, nil)
		...
		},
		{
			...
			req:  &domain.LoginCmd{Email: email, Password: password},
			setupMock: func(deps *common.MockedDeps, authClient *sharedmocks.MockAuthClient) {
				deps.UserRepo.EXPECT().FindByEmail(gomock.Any(), email).Return(&domain.User{ID: "user_123", AuthUserID: &authUserID, EmailVerified: false, Email: email}, nil)
				deps.OutboxRepo.EXPECT().ExistsPendingByReference(gomock.Any(), domain.EventTypeSendVerificationEmail, "user_123").Return(false, nil)
				deps.OutboxRepo.EXPECT().Create(gomock.Any(), gomock.Any()).DoAndReturn(
					func(_ context.Context, event *domain.OutboxEvent) error {
						require.NotNil(t, event)
						assert.Equal(t, domain.EventTypeSendVerificationEmail, event.Type)
		...
		},
		{
			...
			setupMock: func(deps *common.MockedDeps, authClient *sharedmocks.MockAuthClient) {
				...
				deps.UserRepo.EXPECT().FindByEmail(gomock.Any(), email).Return(&domain.User{ID: "user_123", AuthUserID: nil, EmailVerified: true, Email: email}, nil)
				deps.UserRepo.EXPECT().GetPasswordHash(gomock.Any(), email).Return(string(hash), nil)
				deps.OutboxRepo.EXPECT().ExistsPendingByReference(gomock.Any(), domain.EventTypeMigrateLegacyUser, "user_123").Return(false, nil)
		...
		},
		{
			...
			req:  &domain.LoginCmd{Email: email, Password: "wrong_password"},
			setupMock: func(deps *common.MockedDeps, authClient *sharedmocks.MockAuthClient) {
				deps.UserRepo.EXPECT().FindByEmail(gomock.Any(), email).Return(&domain.User{ID: "user_123", AuthUserID: &authUserID, EmailVerified: true, Email: email}, nil)
...
func TestAuthService_ResetPassword(t *testing.T) {
	tests := []struct {
		name      string
		req       *domain.ResetPasswordCmd
		setupMock func(authClient *sharedmocks.MockAuthClient)
		wantErr   error
	}{
		{
			name: "success",
			req:  &domain.ResetPasswordCmd{Token: "valid", Password: "NewPassword123!"},
			setupMock: func(authClient *sharedmocks.MockAuthClient) {
		...
		{
			name: "invalid_token",
			req:  &domain.ResetPasswordCmd{Token: "invalid", Password: "NewPassword123!"},
			setupMock: func(authClient *sharedmocks.MockAuthClient) {
...
func TestAuthService_VerifyEmail(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(deps *common.MockedDeps, authClient *sharedmocks.MockAuthClient) {
				...
				deps.UserRepo.EXPECT().FindByAuthUserID(gomock.Any(), "auth_id").Return(&domain.User{ID: "user_123", CompanyID: companyID, Email: email, EmailVerified: false}, nil)
				deps.BillingSvc.EXPECT().ProvisionSignupCredits(gomock.Any(), companyID).Return(nil)
		...
		},
		{
			...
			setupMock: func(deps *common.MockedDeps, authClient *sharedmocks.MockAuthClient) {
				authClient.EXPECT().VerifyEmailWithOTP(gomock.Any(), email, token).Return("access_token", &sharedauth.AuthUserInfo{Sub: "auth_id", Email: email}, nil)
				deps.UserRepo.EXPECT().FindByAuthUserID(gomock.Any(), "auth_id").Return(&domain.User{ID: "user_123", CompanyID: companyID, Email: email, EmailVerified: true}, nil)
				deps.UserRepo.EXPECT().UpdateEmailVerified(gomock.Any(), "user_123", true).Return(nil)
				deps.UserRepo.EXPECT().UpdateLastLogin(gomock.Any(), "user_123").Return(nil)
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := deps.NewAuthService(pool, authClient, nil, nil, []byte(testEncryptionKey))
			_, err := svc.VerifyEmail(context.Background(), &domain.VerifyEmailCmd{Email: email, Token: token})
