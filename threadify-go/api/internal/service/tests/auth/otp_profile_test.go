...
func TestSignupOTPThenProfile(t *testing.T) {
	for _, mode := range []string{"no_jwks", "symmetric_signing", "asymmetric_signing"} {
		t.Run(mode, func(t *testing.T) {
			...
			require.NoError(t, err)
			svc := deps.NewAuthService(nil, client, nil, []byte(testEncryptionKey))
			if mode != "no_jwks" {
				svc.SetJWKSVerifier(sharedauth.NewJWKSVerifier(issuer+"/.well-known/jwks.json", "authenticated", issuer))
			}
