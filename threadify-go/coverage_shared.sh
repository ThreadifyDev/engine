...
# Run tests with coverage for all shared packages
PACKAGES=$(go list ./...)

# Run tests for each shared package
...
if ls coverage_*.out 1> /dev/null 2>&1; then
    ...
    if [ -s coverage.out ] && [ "$(wc -l < coverage.out)" -gt 1 ]; then
        ...
        echo "Coverage report generated: shared/coverage.html"
    else
