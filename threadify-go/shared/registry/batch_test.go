...
func TestBatchUsageIsAtomicAcrossMetricsAndReplicas(t *testing.T) {
	...
	enableTestMeter(t, r)
	other := &Runtime{cfg: r.cfg, accountID: r.accountID, pool: pool}
	enableTestMeter(t, other)
	now := time.Date(2026, 9, 15, 12, 0, 0, 0, time.UTC)
	// More contenders than either allowance, using both metric orders and separate runtimes.
 ...
 (SELECT count(*) FROM threadify_registry_outbox)`, InputBytes, InputRequests).Scan(&bytes, &requests, &outbox); err != nil {
	...
	if bytes != 30 || requests != 10 || outbox != 2 {
...
func TestBatchUsageStorageFailureAndOverflow(t *testing.T) {
	...
	if err := r.ProjectUsage(ctx); err == nil {
		t.Fatal("storage failure acknowledged projection")
	}
	var rows int
	...
	if err := r.Check(ctx, OutputBytes, math.MaxInt64-7); err != nil {
		t.Fatal(err)
	}
