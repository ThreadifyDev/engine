...
2. `threadify.thread_key`: your reference for one logical run, such as `order-123/payment-attempt-2`.
3. `workflow.run_id`: used by default when no explicit thread key is present.
4. OTel trace ID: one trace maps to one thread.

References are case-sensitive strings, trimmed at the edges, with a maximum of 1024 UTF-8 bytes. Explicit thread keys must not be blank; blank workflow run IDs fall through to the trace ID. Explicit thread keys and workflow run IDs use the same identity namespace: the same value selects the same thread. Trace IDs use a separate namespace. All identities are scoped to the licensed company.
...
The endpoint still requires `X-API-Key` and OTLP Protobuf. The query value must be exactly `true` or `false`; repeated or invalid values are rejected. `false` ignores only `workflow.run_id`, so an explicit `threadify.thread_key` still works.
...
Terminal threads reject further writes. Initialize contracted sessions before reporting spans; resuming uses the stored contract version, and a different supplied contract is rejected.
