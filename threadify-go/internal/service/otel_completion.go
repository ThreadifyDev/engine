...
func isOwnedOTelThread(thread *domain.Thread, companyID, traceID string) bool {
	return traceID != "" && thread.CompanyID == companyID && thread.ContractName == "" &&
		(thread.ContractID == nil || *thread.ContractID == "") &&
		thread.Refs["otel_trace_id"] == traceID &&
		thread.ID == uuid.NewSHA1(uuid.NameSpaceOID, []byte(companyID+":"+traceID)).String()
}
