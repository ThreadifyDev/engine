...
// ArchiveStepState writes step state snapshot to stream for persistent storage
func (r *ActivityRepository) ArchiveStepState(ctx context.Context, threadID, stepID, stepName, idempotencyKey, status string) error {
	// Get step state from hash
	stepKey := fmt.Sprintf("thread:%s:steps:%s:%s", threadID, stepName, idempotencyKey)
	stepData, err := r.valkey.HGetAll(ctx, stepKey)
	if err != nil {
		return fmt.Errorf("failed to get step state %s: %w", stepKey, err)
	}
	...
	_, err = r.valkey.XAdd(ctx, "streams:thread_step_state", streamValues)
	if err != nil {
		return fmt.Errorf("failed to write step state to stream: %w", err)
	}
