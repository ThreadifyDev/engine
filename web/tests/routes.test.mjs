...
const output = await build({
  stdin: {contents: "export {pages, redirects} from './app/routes';", resolveDir: fileURLToPath(new URL('..', import.meta.url))},
  bundle: true, write: false, format: 'esm', platform: 'node',
  plugins: [{name: 'defer-page-components', setup(b) {
    b.onResolve({filter: /^\.\/routes\//}, args => ({path: args.path, external: true}));
  }}],
});
const {pages, redirects} = await import(`data:text/javascript;base64,${Buffer.from(output.outputFiles[0].text).toString('base64')}`);
