...
export function APIKeysTab() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('create-api-key', handleCreate);
    return () => window.removeEventListener('create-api-key', handleCreate);
  }, []);
  ...
  const getServiceAccountName = (serviceAccountId?: string) => {
    if (!serviceAccountId) return 'N/A';
    const account = serviceAccounts.find(sa => sa.id === serviceAccountId);
              ...
              {apiKeys.map((key, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">
                    {getServiceAccountName(key.service_account_id)}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-600">
