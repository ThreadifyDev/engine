...
export function EngineTab() {
  ...
  return <section className="max-w-3xl space-y-4 text-sm leading-5">
    <h3 className="text-base font-semibold">Engine URL</h3>
    ...
    {!settings ? <p>Loading Engine settings…</p> : <>
      <form onSubmit={save} className="space-y-3">
        ...
        <input id="public-engine-url" type="url" required maxLength={2048} placeholder="https://threadify.example.com" value={url} disabled={!settings.can_manage || busy} onChange={e => setUrl(e.target.value)} className="w-full rounded border px-3 py-2 text-sm" />
        {settings.can_manage && <div className="flex gap-3">
          <button disabled={busy || !url.trim()} className="rounded bg-black px-4 py-2 text-white disabled:opacity-50">Save Engine URL</button>
          <button type="button" disabled={busy || settings.source !== 'ui'} onClick={reset} className="rounded border px-4 py-2 disabled:opacity-50">Use config default</button>
        </div>}
      </form>
