...
// Main step with sub-steps for complex payment flow
const paymentStep = thread.step('payment_processed');

try {
  // Add sub-steps as you execute them
  paymentStep.subStep('validate_card', { 
    cardType: 'visa', 
    last4: '1234' 
  }, 'success');
  
  paymentStep.subStep('check_fraud', { 
    fraudScore: 0.15,
    riskLevel: 'low'
  }, 'success');
  
  paymentStep.subStep('authorize_payment', { 
    authCode: 'AUTH-123',
    amount: 299.99
  }, 'success');
  
  paymentStep.subStep('process_transaction', { 
    transactionId: 'TXN-456',
    processor: 'stripe'
  }, 'success');
  
  // Complete main step with context
  await paymentStep
    .addContext({ 
      totalAmount: 299.99,
      currency: 'USD',
  ...
  paymentStep.subStep('error_handler', { 
    error: error.message,
