...
_LOGGER = get_logger("threadify.extension")
...
class ThreadifySettings:
    ...
    def from_environment(cls, default_service_name: str) -> ThreadifySettings:
        """Read process-owned credentials only when the extension starts."""
...
class ThreadifyDelivery:
    ...
    def __init__(
        self, owner: ThreadifyExtension, notification: Notification
    ) -> None:
...
class ThreadifyContext(ExtensionContext):
    """Typed invocation view exposed through ``context.extensions('threadify')``."""

    __slots__ = ("_owner",)

    def __init__(self, extension_name: str, owner: ThreadifyExtension) -> None:
...
class ThreadifyExtension(Extension[ThreadifyContext]):
    ...
    def create_context(self, base: ExtensionContext) -> ThreadifyContext:
        """Create a fresh revocable SDK view only after connection startup."""

        self._require_connection()
        return ThreadifyContext(base.extension_name, self)
    ...
    def _schedule_delivery(
        ...
        """Give each notification one task whose lifetime the extension owns."""
    ...
    async def _join_thread(
        ...
        connection = self._require_connection()
        async with extension_mutation("threadify", "thread.join", trigger="agent"):
    ...
    async def _record_step(
        ...
        *,
        ...
        async with extension_mutation("threadify", operation, trigger="agent"):
    ...
    async def _complete_thread(
        ...
        async with extension_mutation("threadify", "thread.complete", trigger="agent"):
    ...
    def _require_connection(self) -> Connection:
        ...
        if not self._started or self._connection is None:
            raise RuntimeError("Threadify extension is not started")
